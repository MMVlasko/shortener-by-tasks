...
#### Установка (Windows 10, Docker Desktop, Pycharm)

1) В корне проекта создать файл `.env`:

```
SECRET_KEY="<cекретный ключ Django>"
DEBUG=<True/False> # Вывод отладочных сообщений в браузере
...
4) Запуск консоли контейнера

`docker-compose exec web bash`

5) Создание суперпользовтаеля для доступа к панели администратора Django

`python manage.py createsuperuser`

6) При изменении в моделях базы данных
...
* `/admin` - панель админа, на которой можно просмотреть содержание таблиц базы данных. Вход осуществляется с учётными
данными, указанными на шаге 5.
...
![diagram.png](../shortener/diagram.png)
...
Либо ошибки в формате (здесь и далее)
...
Авторизация производится на основе сессий. После авторизации у клиента сохраняются cookie:
`csrftoken` и `sessionid`. Проверка авторизации производится автоматически, как и получение данных пользователя (username и т. д.)
при авторизованных запросах. `csrftoken` нужно передавать в заголовке `X-CSRFTOKEN` при POST-запросах;
в DEBUG-режиме, для удобства работы со Swagger эта необходимость отключена (см. shortener/settings.py и shortener/middleware.py).


-- `/api/users/logout/` `POST` - выход авторизованного пользователя при запросе происходит автоматически.
